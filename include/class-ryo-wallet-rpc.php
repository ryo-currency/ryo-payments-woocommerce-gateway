 ...
 * Modified to work with ryo-wallet-rpc wallet by mosu-forge
 */

defined( 'ABSPATH' ) || exit;

class Ryo_Wallet_Rpc
{
    ...
    protected function & getResponse(&$pRequest)
    {
        ...
        if (0 < curl_errno($ch)) {
            if(is_admin())
                echo '[ERROR] Failed to connect to ryo-wallet-rpc at ' . $this->host . ' port '. $this->port .'</br>';
        }
    ...
    /*
     * The following functions can all be called to interact with the Ryo RPC wallet
    ...
    public function make_uri($address, $amount, $recipient_name = null, $description = null)
    {
        ...
        $new_amount = $amount * RYO_GATEWAY_ATOMIC_UNITS_POW;
