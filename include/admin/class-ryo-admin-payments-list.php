...
class Ryo_Admin_Payments_List extends WP_List_Table {
    ...
    function extra_tablenav($which) {
        if ($which == "top") {
            ...
            if(Ryo_Gateway::get_confirm_type() == 'ryo-wallet-rpc') {
                $balance = Ryo_Gateway::admin_balance_info();
                $balance_info = <<<HTML
...
HTML;
            ...
            echo <<<HTML
            <div class="wrap">
                <h1 class="wp-heading-inline">Ryo Payments</h1>
                $balance_info
                <hr class="wp-header-end">
                <ul class="subsubsub">
                    <li>
                        <a href="?page=ryo_gateway_payments&type=all" {$tab_info['all']['active']}>
                            All <span class="count">({$tab_info['all']['count']})</span>
                        </a> |
                    </li>
                    <li style="display:none">
                        <a href="?page=ryo_gateway_payments&type=pending" {$tab_info['pending']['active']}>
                            Pending <span class="count">({$tab_info['pending']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=ryo_gateway_payments&type=paid" {$tab_info['paid']['active']}>
                            Received <span class="count">({$tab_info['paid']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=ryo_gateway_payments&type=confirmed" {$tab_info['confirmed']['active']}>
                            Confirmed <span class="count">({$tab_info['confirmed']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=ryo_gateway_payments&type=expired" {$tab_info['expired']['active']}>
                            Expired <span class="count">({$tab_info['expired']['count']})</span>
                        </a>
                    </li>
                </ul>
                <form id="ryo-payments-filter" method="get" style="display:none">
                    <p class="search-box">
                ...
                <h2 class="screen-reader-text">Ryo Payments List</h2>
                <style>
...
HTML;
    ...
    public function column_default($item, $column_name) {
        ...
        case 'col_payment_id':
            ...
            break;
        case 'col_txid':
            $url = RYO_GATEWAY_EXPLORER_URL.'/tx/'.$item->txid;
        ...
        case 'col_height':
            ...
            break;
        case 'col_amount':
            echo Ryo_Gateway::format_ryo($item->amount).' Ryo';
            break;
        }
    }
    ...
    public function no_items() {
        esc_html_e('No Ryo payments found', 'ryo_gateway');
    }
